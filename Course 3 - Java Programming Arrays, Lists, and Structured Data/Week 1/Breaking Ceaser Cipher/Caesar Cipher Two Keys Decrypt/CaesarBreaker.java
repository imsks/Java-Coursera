...
/**
 * Idea for two keys decrypt method. Recall that in using two keys, 
 * key1 and key2, key1 was used to encrypt every other character, 
 * starting with the first, of the String, and key2 was used to encrypt 
 * every other character, starting with the second. In order to decrypt 
 * the encrypted String, it may be easier to split the String into two 
 * Strings, one String of all the letters encrypted with key1 and one String 
 * of all the letters encrypted with key2. Then use the algorithm from the 
 * lesson to determine the key for each String, and then use those keys and 
